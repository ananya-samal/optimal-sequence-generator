...
# Adding a conditional check to verify user input a natural number only

input_new = ''  #Initialising it just to be sure while loop never to run into indefinite loop 
while True:
    input_new = input('Enter a number to generate series :') #Taking input from user
    if float(input_new) != 0 and input_new.isdigit():
            break
    else:
        print('!!! INVALID INPUT !!! Enter a Natural number only to generate series')
